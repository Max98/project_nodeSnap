...
GridSnap.prototype.getNearestVertexFromEvent = function(event) {
    ...
    const intersects = this.raycaster.intersectOctreeObject(
        this.intersectMesh,
        true
    );
    ...
    if (intersects.length > 0) {
        ...
        const closestVertex = this.octree.findClosestVertex(
            nearestPoint,
            this.radius
        );
...
GridSnap.prototype.addOrRemoveMarker = function(vertex) {
    ...
    const markerMesh = this.markerMesh.clone();
...
GridSnap.prototype.mouseUp = function(event) {
    ...
    const nearestVertex = this.getNearestVertexFromEvent(event);
    if (nearestVertex) {

...
export default class TruckFileImporter {
   ...
   * @returns truckTitle
   */
  async loadFile(filePatch: string) {
    const txt = await fs.readTextFile(filePatch);

    this.FileBuffer = txt.split("\n");
  ...
  private ParseNodesUnified() {
    ...
    if (this.currArgs[4]) {
       ...
       */
      if (this.currArgs[5]) {
        nodeParams.options = nodeParams.options + " " + this.currArgs[5];
      }
    }
  ...
  private ParseGroup() {
    ...
    const groups: TruckSectionsInterface.TruckFileGroup = {
      grpId: this.truckFile.groups.length,
  ...
  private ParseFileinfo() {
    ...
    if (this.truckFile.fileinfo == undefined) {
      ...
      if (this.currArgs[2]) {
        this.truckFile.fileinfo.categoryId = parseFloat(this.currArgs[2]);

        if (this.currArgs[3]) {
          this.truckFile.fileinfo.fileVersion = parseFloat(this.currArgs[3]);
        }
      }
    }
  ...
  private IdentifyKeywordInCurrentLine(): Keyword {
    ...
    console.log(
      "Found keyword: KEYWORD_" + this.lineSplit(this.currLine.toUpperCase())[0]
    );
    ...
    const searchKeyword: any =
      Keyword[
        ("KEYWORD_" + this.lineSplit(this.currLine.toUpperCase())[0]) as any
      ];
    ...
    if (searchKeyword != undefined) {
      ...
      console.warn("Keyword not supported, parsing as unknown...");
  ...
  private ChangeSection(newSection: Section, bl = false) {
    if (!bl) console.info("Changing section to: " + Section[newSection]);
